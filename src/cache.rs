...
struct Inner {
    ...
    cache: RwLock<
        Vec<(TextureFormat, MultisampleState, Option<DepthStencilState>, Arc<RenderPipeline>)>
    >,
}
...
impl Cache {
    pub fn new(device: &Device) -> Self {
        ...
        let atlas_layout = device.create_bind_group_layout(
        ...
        let uniforms_layout = device.create_bind_group_layout(
            &(wgpu::BindGroupLayoutDescriptor {
                entries: &[
                    BindGroupLayoutEntry {
                        binding: 0,
    ...
    pub(crate) fn get_or_create_pipeline(
        ...
        let Inner { cache, pipeline_layout, shader, vertex_buffers, .. } = self.0.deref();
