...
impl TextRenderer {
    ...
    pub fn prepare_with_depth<'a>(
        ...
        for text_area in text_areas {
            ...
            let layout_runs = text_area.buffer
                .layout_runs()
            ...
            for run in layout_runs {
                for glyph in run.glyphs.iter() {
                    ...
                    if atlas.mask_atlas.glyph_cache.contains(&physical_glyph.cache_key) {
                        atlas.mask_atlas.promote(physical_glyph.cache_key);
                    } else if atlas.color_atlas.glyph_cache.contains(&physical_glyph.cache_key) {
                        ...
                        inner.put(physical_glyph.cache_key, GlyphDetails {
                            width: width as u16,
                    ...
                    let mut y =
                        ((run.line_y * text_area.scale).round() as i32) +
                        physical_glyph.y -
                        (details.top as i32);
...
fn create_oversized_buffer(
    ...
    buffer
        ...
        [..contents.len()].copy_from_slice(contents);
    buffer.unmap();
    (buffer, size)
}
